...
alias powersave='cpufreq-set -r -g powersave'

alias performance='cpufreq-set -r -g performance'
...
alias irdgml='cat "$HOME"/mphil/irdg/irdg-mailing-list.txt'

alias shuffle-videos='mp -shuffle $(find "$HOME"/videos/music/* -type f)'
...
export PATH="$PATH:~/.julia/julia/:~/.gem/ruby/1.9.1/bin"

export LS_COLORS

export CDPATH='.:~:~/mphil:~/rstat:~/mighty:~/tmp:~/tmp/perm:~/programming:~/images:~/videos:~/music/:~/poetry:/run/media/akhilsbehl'

export GREP_COLOR='1;31'

if [[ $USER == "root" ]]; then
  export PS1='\[\e[31;1m\](\w) ROOT# \[\e[30;0m\]'
else
  export PS1='\[\e[34;1m\](\w) TheArchWizard$ \[\e[30;0m\]'
fi
...
alias menu='sudo $EDITOR /boot/grub/menu.lst &'
