...
7. **Engine Format Adapters & Authority Rules**:
   - Translates generic launch arguments (`model`, `thinking`, `systemPrompt`, `context`) into native CLI options:
     ...
     - `agy`: `--model`, `--effort` (`low`|`medium`|`high`; requested values outside this set are recorded in launch metadata but omitted from the launch command, per item 11 below), `-p`/`--print`/`--prompt` (all aliases of the same non-interactive single-prompt flag — verified against a real `agy --help`; `-i`/`--prompt-interactive` is a distinct interactive-continuation mode, not a way to pass prompt text, and is not used by `pi-subagents`). The exact argument-passing shape (positional prompt vs. flag value) needs a live smoke test before this adapter is implemented.
...
8. **Primary Dispatch Guard**:
   - A `pi.on("tool_call")` hook, gated on `event.toolName === "bash"`, that classifies the command's target binary — not the presence of backgrounding syntax. A generic backgrounded command (`npm test &`, `tail -f log.txt &`) is always allowed; the guard fires only when the command directly invokes `pi`, `claude`, `codex`, or `agy` (including flag variants like `claude --bg ...`), since all four bypass the fleet's IPC/worktree/supervision tracking identically when launched outside `subagents_launch`.
   - Denies the call and instructs the model to use `subagents_launch` instead.
   - No firstmate precedent exists for this classifier: firstmate's own delegation guard works by matching the *tool's name* against known delegation-tool stems, which only applies to harnesses exposing distinct native delegation tools (Claude's `Task`, Codex's equivalent) — Pi has no such tool, everything is the flat `bash` tool, so this had to be designed fresh rather than adopted (see Ticket 10's correction in `.scratch/firstmate-deep-scan/`).
   ...
   - **Implementation guidance**: the hook-wiring mechanism (gate on `event.type === "tool_call"`, extract `event.input.command`, return `{ block: true, reason }` to veto before execution) is validated against Pi 0.80.5 by Firstmate's cd-guard and watcher-arm-guard, both wired through this same handler shape in `.pi/extensions/fm-primary-turnend-guard.ts` — reuse that wiring. The command-classification logic itself (which binary is the command's target) has no Firstmate precedent and is specified above.
...
- **Seams Tested**:
  1. *Engine Argument Adapters*: Pure function unit tests verifying CLI flag translation for `pi`, `claude`, `codex`, `agy`. `agy`'s adapter additionally needs a live smoke test against the real `agy` binary to confirm the exact argument-passing shape (positional prompt vs. flag value) before the unit tests are written, since its flags were previously assumed rather than verified.
  2. *Runner IPC & Control Plane*: Unit/component tests verifying `status.json` serialization, `unknown` fail-closed handling, control vs prompt command separation in `inbox.jsonl`, `Ctrl+U` buffer clearing post-interrupt, and stream prompt pattern matching. The prompt patterns themselves (what a permission prompt vs. an authority question literally looks like on-screen) need a live smoke test per engine to capture real text before the pattern-matching unit tests are written — no existing source (including firstmate) has this, since it's specific to each CLI's own rendering.
  ...
  8. *Primary Dispatch Guard Classifier*: Unit tests with an explicit true/false-positive matrix — true positives: `claude -p ...`, `claude --bg ...`, `codex exec ...`, `agy -p ...`, `pi ...` invoked directly via `bash`; false positives that must stay allowed: `npm test &`, `tail -f log.txt &`, any backgrounded command whose target binary is not one of the four engines.
...
- `.scratch/firstmate-deep-scan/map.md`'s "Not yet specified" section carries three deferred items (fleet-navigation shortcuts, context-pruning adapter, agy/codex busy-signal verification) that are gated on this spec being implemented, not on more research. When splitting this spec into tickets, carry those three items and their trigger conditions forward rather than dropping them — each becomes live only once its trigger (named in the map) is hit during implementation.
