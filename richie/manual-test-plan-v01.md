...
This version corrects the test-target and expected-rendering errors found during
the v00 run. The v00 run remains valid evidence. Do not repeat cases already
recorded as passing solely because this plan has a new version.

## Test setup and evidence rules

Use a copy of this file named `manual-test-run-v01.md` if you need to repeat the
run. The clean baseline must remain available as `manual-test-plan-v01.md`.

Before the first case:

1. Run `npm run check`, `npm test`, and `npm run build`.
2. Start the service with `npm start`.
3. Open this file with `npm run review -- manual-test-plan-v01.md`.
4. Confirm the page uses the light Rosé Pine Dawn presentation. There should be
   no dark-mode control.
5. Confirm `Document level note`, `Abort review`, and `Finish review` are equal
   width, vertically stacked at the top of the right sidebar.
6. Confirm the left sidebar contains the user-guide link, search controls, and a
   ...
   feedback message.
7. Confirm both sidebars align near the top of the viewport and have visible
   horizontal space between them and the document.
8. Keep the browser developer console open. Record any error, warning, failed
...
## Automated coverage and rerun policy
...
The v00 evidence remains the full manual baseline. Do not rerun that suite. Escape
dismissal, native browser context-menu suppression, and final visual placement are
browser interactions not exercised by the current Node test harness. Treat them as
targeted smoke checks only if a future browser-automation harness becomes available
or the affected interaction changes again.
...
4. Confirm numbered Markdown lists render with numbers and unordered lists render
   with bullets.
5. Confirm ordinary prose, *emphasis*, **strong text**, ~~strikethrough~~,
...
6. Hover the link `the company noticeboard`. Confirm it changes color without
   opening a new page unless clicked.
7. Hover the first table row and the code block. Confirm the page does not add
...
2. Confirm the page scrolls that heading near the top of the reading area without
   placing it under either sidebar.
...
does not create feedback, change the source, or open a new tab. The target heading
is not hidden above the viewport. At narrow width, the sidebars become normal
stacked sections.
...
8. Press `Escape` while the menu is visible. Confirm it closes without feedback.
   Keep the pointer still and confirm a pending hover timer does not reopen it.
...
Under `Fixture: inline, block, list, and table text`, select exactly `The second
sentence makes block selection visibly different from a sentence selection.`
Choose `Delete`.
...
Under `Distinct blocks`, hover the paragraph beginning `This is block A.` Choose
`Delete` from the hover menu, not from a text selection.
...
Hover the `Fixture: thematic break and final paragraph` heading. Choose `Delete`.
...
Under `Distinct blocks`, hover either `Block D` list item and choose `Delete list`.

Expected: the list block is targeted as one block. The card is not a range operation
for whichever list item happened to be under the pointer. The ordinary `Delete`
action remains available for deleting only the hovered item paragraph.
...
1. Under `Distinct inline targets`, select `alpha word` and press `c`. Enter
   `Comment using the keyboard.`
2. In the same fixture, select `beta phrase` and press `r`. Enter
   `replacement phrase`.
3. In the same fixture, select `gamma sentence` and press `d`.
...
7. Open a comment dialog and press `Escape`. Confirm it cancels the dialog without
   saving feedback.
8. With a document selection active, right-click once. Confirm Richie's floating
   menu remains the active action surface and the browser context menu does not
   take keyboard focus.
...
Richie suppresses the browser context menu only while a valid document selection
exists. Normal right-click behavior remains available when no text is selected.
...
Expected: the card scope is `cell`, with the cell quote and replacement. The old
cell text is struck through and a visible `Replacement:` line shows the proposed
text. The rendered table remains structurally intact.
...
### MT-10C. Optional table-styling regression

Run this only after table or review-highlight styling changes. With the column
operation open, comment on a cell in another column, then hover each row.

Expected: the original column remains identifiable, the separately commented cell
has its own target styling, and row hover tint does not erase either indication.
This case protects overlapping CSS states. It is not required for routine review
...
Open the `Fixture: invalid Mermaid diagram` section near the end. Confirm the
invalid diagram shows a readable inline `Mermaid did not render` warning. Confirm
its disclosure is automatically open and labelled `Mermaid source (render failed)`. Select
`this is not valid Mermaid`. Add the comment:
...
1. Confirm the original `manual-test-plan-v01.md` has no `<<ASB:` markers.
...
4. Confirm deletion markers quote the exact selected source and distinguish cells,
